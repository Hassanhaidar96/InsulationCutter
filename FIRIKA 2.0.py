...
def get_centers_Length(num_ribs, Length):
    ...
    if num_ribs == 1:
        return [first_center] if Length >= first_center + min_end_margin else []

    max_last_center = Length - min_end_margin

    # Step 1: Find the farthest last_center from first_center in 100 mm steps
    last_center = first_center
    while last_center + 100 <= max_last_center:
        last_center += 100

    # Step 2: Place first and last ribs
    centers = [first_center, last_center]

    # Step 3: Fill in (num_ribs - 2) ribs in between, as evenly as possible on the 100 mm grid
    available_positions = [round(first_center + i * 100, 1) for i in range(1, int((last_center - first_center) // 100))]
    if len(available_positions) < num_ribs - 2:
        return [] # Not enough positions between first and last

    # Pick evenly spaced positions from available
    step = len(available_positions) / (num_ribs - 2)
    for i in range(num_ribs - 2):
        index = round(i * step)
        centers.insert(-1, available_positions[index])
