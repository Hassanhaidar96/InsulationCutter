...
if not st.session_state.logged_in:
    ...
    st.stop()  # This will stop the rest of the app from running
...
def create_dxf(elements_data):
    ...
    for element in reversed(elements_data): # To have the drawings in correct order
        ...
        msp.add_text(code, dxfattribs={
            'height': 10,
            'insert': (text_x, text_y),  # Insert point
        })
...
num_elements = st.number_input('Anzahl der Elemente', min_value=1, max_value=20, value=1, step=1)

log_action(f'number of elements = {int(num_elements)}')
