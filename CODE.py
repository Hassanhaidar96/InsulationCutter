...
def adjust_h_for_fire_resistance(h, fire_resistance):
...
def get_centers_1m(num_ribs):
    if num_ribs == 2:
        ...
        return [64.5 + i*100 for i in range(10)]
    else:
        return []
...
# Streamlit UI
st.title('DXF Generator for Slab Element')
...
fire_resistance = st.selectbox('Fire resistance', ['R0', 'REI60', 'REI90', 'REI120'])
insulation = st.radio('Insulation material', ['SW', 'EPS'])

# Process inputs
adjusted_h = adjust_h_for_fire_resistance(h_rib, fire_resistance)
